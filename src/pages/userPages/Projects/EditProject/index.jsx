    ...
    const fetchData = async () => {
      try {
        ...
        setFormData({
          ...
          existingImageNames:
            projectData.project_image?.map((img) => img.name_image) || [],
          imagesToDelete: [],
          teamMembers: projectData.team?.length > 0 ? projectData.team : [],
        });

        if (projectData.technology) {
          const techs = projectData.technology
            .split(",")
            .map((t) => t.trim())
            .filter((t) => t);
  ...
  const handleImageChange = (e) => {
    ...
    const validFiles = files.filter((file) => {
      if (file.size > maxSize) {
        Swal.fire(
          "Gagal",
          `${file.name} terlalu besar. Maksimal ukuran 2MB.`,
          "error"
        );
        return false;
      }
    ...
    if (
      ...
      Swal.fire(
        "Error",
        "Kamu hanya bisa mengunggah maksimal 2 gambar.",
        "error"
      );
      return;
    }
    ...
    setFormData((prev) => ({
      ...
      imageNames: [
        ...prev.imageNames,
        ...validFiles.map((file) => file.name.split(".")[0]),
      ],
    }));
  };
    ...
    try {
      ...
      formData.existingImages.forEach((image, index) => {
        ...
        formDataToSend.append(
          `existing_images[${index}][name_image]`,
          formData.existingImageNames[
      ...
      Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Karyamu berhasil diperbarui!",
        confirmButtonText: "OK",
      }).then(() => {
        navigate(`/project/${slug}`);
      });
    } catch (error) {
      console.error("Gagal memperbarui karya:", error);
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="space-y-1">
              ...
              <p id="techHelp" className="text-xs text-gray-500 mt-1">
                Pisahkan setiap teknologi dengan koma atau tekan Enter. Klik ✕
                untuk menghapus.
              </p>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                {formData.existingImages
                  ...
                  .map((image, index) => (
                    <div
                      key={image.id}
                      className="border rounded-lg p-3 relative group bg-white"
                    >
                      ...
                      <input
                        ...
                        value={
                          formData.existingImageNames[
                            formData.existingImages.findIndex(
                              (img) => img.id === image.id
                            )
                          ] || ""
                        }
                        onChange={(e) =>
                          handleExistingImageNameChange(
                            formData.existingImages.findIndex(
                              (img) => img.id === image.id
                            ),
                            e.target.value
              ...
              <label className="flex flex-col items-center justify-center w-full border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100 transition-colors">
                <div className="flex flex-col items-center justify-center py-8 px-2">
                  ...
                  <p className="mb-2 text-sm text-center text-gray-500">
                    ...
                    atau seret dan masukan
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                    PNG, JPG ukuran 2MB (max 2 images)
                  </p>
                  <p className="mt-1 text-sm text-gray-500">
                    Resolusi terbaik minimal 1000x700px
                  </p>
                </div>
                <input
                  type="file"
                ...
                {formData.images.map((image, index) => (
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleNewImageNameChange(index, e.target.value)
                      }
        ...
        <div className="space-y-6">
          <div className="space-y-4">
            ...
            {formData.teamMembers.map((member, index) => (
              <div
                key={index}
                className="grid md:grid-cols-12 grid-cols-1 gap-3 items-end bg-gray-50 p-4 rounded-lg"
              >
        ...
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-8">
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-2">
              Perbarui Karya <br />
              <span className="text-OxfordBlue">{formData.title}</span>
            </h1>
            <p className="text-lg text-gray-600 max-w-2xl mx-auto">
              Perbarui detail Karya dan buat karya ajaibmu lebih menarik
            </p>
          </div>
          ...
          <div className="bg-white shadow-lg rounded-xl overflow-hidden border border-gray-200 transition-all hover:shadow-xl">
            ...
            {/* Form Content */}
            <div className="p-6 sm:p-8">{steps[activeStep].content}</div>
